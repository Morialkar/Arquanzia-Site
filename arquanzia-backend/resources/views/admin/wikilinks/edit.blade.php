<x-layouts.admin title="Modifier wikilink">
    <div class="mb-6">
        <a href="{{ route('admin.wikilinks.index') }}" class="text-arq-forest hover:text-arq-forest-light">← Retour aux wikilinks</a>
    </div>

    <h1 class="font-serif text-2xl font-bold text-arq-forest mb-6">Modifier wikilink</h1>

    <form action="{{ route('admin.wikilinks.update', $wikilink) }}" method="POST" class="bg-arq-parchment rounded-xl shadow-sm border border-arq-amber/20 p-6 max-w-2xl">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="term" class="block text-arq-bark font-medium mb-1">Terme <span class="text-red-500">*</span></label>
            <input type="text" name="term" id="term" value="{{ old('term', $wikilink->term) }}" required
        ...
        <div class="mb-4">
            ...
            <select name="encyclopedia_node_id" id="encyclopedia_node_id"
                ...
                @foreach($nodes as $node)
                    <option value="{{ $node->id }}" {{ old('encyclopedia_node_id', $wikilink->encyclopedia_node_id) == $node->id ? 'selected' : '' }}>
                        {{ $node->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-6">
            ...
            <input type="url" name="custom_url" id="custom_url" value="{{ old('custom_url', $wikilink->custom_url) }}"
                class="w-full px-4 py-2 border border-arq-amber/30 rounded-lg bg-white focus:ring-2 focus:ring-arq-forest/20 focus:border-arq-forest"
                placeholder="https://...">
